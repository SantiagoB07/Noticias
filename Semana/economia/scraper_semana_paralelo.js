...
(async () => {
  try {
    ...
    const mainUrl = 'https://www.semana.com/economia/macroeconomia/';
    ...
    let iteration = 0;
    const maxIterations = 20; // Aumenté el límite de iteraciones para cargar más noticias
    ...
    while (loadMoreButtonExists && iteration < maxIterations) {
      ...
      const isButtonVisible = await mainPage.evaluate(() => {
        const button = document.querySelector('.styles__DivVerMas-sc-o51gjq-2.EUsLC');
